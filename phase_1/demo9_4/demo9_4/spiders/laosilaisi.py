...
import logging
import json
from demo9_4.items import Demo94Item
...
class LaosilaisiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in item_list:
            ...
            # 价格
            price = item.xpath(
                './/span[@class = "pirce"]//text()').extract_first()
            ...
            item_instance = Demo94Item()

            item_instance["model"] = model
            item_instance["price"] = price
            item_instance["dist"] = dist
            item_instance["city"] = city
        ...
        next_url = response.xpath(
            '//a[@class="page-item-next"]/@href').extract_first()
        if next_url:
